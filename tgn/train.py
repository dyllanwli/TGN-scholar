...
def train(model, dataloader, sampler, criterion, optimizer, args):
    ...
    for _, positive_pair_g, negative_pair_g, blocks in dataloader:
        ...
        if wandblog:
            wandb.log({"batch_time":  time.time()-last_t})
        # print("Batch: ", batch_cnt, "Time: ",)
        last_t = time.time()
        batch_cnt += 1
    return total_loss
...
if __name__ == "__main__":
    ...
    try:
        if wandblog:
            wandb.init(project="tgn_dscompetition", group="test")
        for i in tqdm(range(args.epochs)):
            ...
            if wandblog:
                wandb.log({
                    "epoch_training_loss": train_loss,
                    "validation_ap": val_ap,
                    "auc": val_auc,
                    "test_ap": test_ap,
                    "test_auc": test_auc,
                    "test_new_node_ap": nn_test_ap,
                    "test_new_node_auc": nn_test_auc
                })
